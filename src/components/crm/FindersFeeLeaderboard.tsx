...
import { Search, Loader2, Medal } from 'lucide-react';
...
interface FindersFeeLeaderboardProps {
  clusterId: string;
}

export function FindersFeeLeaderboard({ clusterId }: FindersFeeLeaderboardProps) {
  ...
  const loadLeaderboard = async () => {
    ...
    const { data: members } = await supabase
      .from('deal_members')
      .select('deal_id, profile_id, compensation_type, compensation_value, is_finder_bonus, profiles:profile_id(id, full_name, avatar_url)')
      .in('deal_id', deals.map(d => d.id))
      .eq('is_finder_bonus', true);
    ...
    members.forEach(m => {
      ...
      const finderFee = m.compensation_type === 'fixed'
        ? m.compensation_value
        : (dealValue * m.compensation_value / 100);

      if (finderFee <= 0) return;
      ...
      memberTotals[m.profile_id].total += finderFee;
    ...
    return (
      <div className="glass-panel p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="font-semibold">Finders Fee Leaderboard</h3>
        </div>
        <p className="text-sm text-muted-foreground text-center py-8">
          No finders fee data yet. Mark deal members as finder to see them here.
        </p>
      </div>
    );
  }
