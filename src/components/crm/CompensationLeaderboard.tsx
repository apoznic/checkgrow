...
import { Trophy, DollarSign, TrendingUp, Loader2, Medal } from 'lucide-react';
...
interface CompensationLeaderboardProps {
  clusterId: string;
}

export function CompensationLeaderboard({ clusterId }: CompensationLeaderboardProps) {
  ...
  useEffect(() => {
    ...
    // Real-time subscription for deal_members changes
    const membersChannel = supabase
      .channel(`leaderboard-members-${clusterId}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'deal_members' }, () => loadLeaderboard())
      .subscribe();

    const dealsChannel = supabase
      .channel(`leaderboard-deals-${clusterId}`)
  ...
  const loadLeaderboard = async () => {
    ...
    // Get all deals for this cluster
    ...
    // Get all deal members
    const { data: members } = await supabase
      .from('deal_members')
      .select('deal_id, profile_id, compensation_type, compensation_value, monthly_amount, equity_percentage, is_finder_bonus, profiles:profile_id(id, full_name, avatar_url)')
    ...
    // Calculate compensation per member
    ...
    members.forEach(m => {
      ...
      const oneTime = (m as any).one_time_commission || 0;
    ...
    return (
      <div className="glass-panel p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="font-semibold">Compensation Leaderboard</h3>
        </div>
        <p className="text-sm text-muted-foreground text-center py-8">
          No compensation data yet. Add members to deals to see the leaderboard.
        </p>
      </div>
    );
  }
