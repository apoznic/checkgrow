...
const pick = (payload: Payload, keys: string[]): { key: string; value: string } | null => {
  const lower = new Map(Object.keys(payload).map((k) => [k.toLowerCase().replace(/[\s-]+/g, "_"), k]));
  for (const k of keys) {
...
Deno.serve(async (req) => {
  ...
  // ---- Deal ----
  const extras = Object.entries(flatten(payload))
  ...
  await Promise.all([
    supabase.from("crm_webhook_events").insert({
      webhook_id: hook.id, cluster_id: hook.cluster_id, external_id: externalId,
  ...
  return json({ ok: true, deal_id: deal.id, contact_id: contactId, source });
});
